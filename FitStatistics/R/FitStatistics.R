...
FitStatistics <- function(y, p, r = NULL, fits){
  ...
  if("rmse" %in% fits){
    RMSE <- apply(e, MARGIN = 2, function(e) sqrt(sum(e^2)/n))
    output <- cbind(output, RMSE)
  }
  # 2) MAD(median absolute deviation):
  if("mad" %in% fits){
    MAD <- apply(e, MARGIN = 2, function(e) median(e))
    output <- cbind(output, MAD)
  }
  # 3) RMSLE
  if("rmsle" %in% fits){
    p.new <- apply(p, MARGIN = 2, function(p) na.omit(p)) # new p without Nas
    ...
    RMSLE <- apply(P, MARGIN = 2, function(P) sqrt(sum((log(P+1)-log(Y+1))^2)/n))
    output <- cbind(output, RMSLE)
  }
  # 4) MAPE
  if("mape" %in% fits){
    MAPE <- apply(a, MARGIN = 2, function(a) sum(a)/n)
    output <- cbind(output, MAPE)
  }
  # MEAPE(median absolute percentage error):
  if("meape" %in% fits){
    MEAPE <- apply(a, MARGIN = 2, function(a) median(a))
    output <- cbind(output, MEAPE)
  }
  # MRAE
  if("mrae" %in% fits){
    if(is.null(r)){
      warning("r must be specified")
    } else {
      b <- as.matrix(abs(r-y)) # find b = |r-y|
      b <- as.matrix(b[!is.na(error[ ,1])])
      b[b==0 & !is.na(b)] <- b[b==0 & !is.na(b)]+0.0001 # prevent the case where b=0  
      MRAE <- apply(e, MARGIN = 2, function(e) median(e/b)) # calculate the fit
